...
<body>
	<?php
			...
			echo('<b style="font-size: 20px">(PRESS CTRL + mouse click for multiple selection)</b></br>');
			//retrieve relevant link	
			echo('<form action="pass_link_to_zip.php" method="post"><select name="match_values_1[]" multiple>');
			foreach ($link_tags as $tag) {
				...
				if(!empty($matches)){
					...
					foreach ($matches as $match) {
						...
						echo('<option value="'.$match[0].'">'.$match[0].'</option>');
			...
			echo('<input type="submit" id="downloadbutton" name="submit" value="Download"/></form><br>');
